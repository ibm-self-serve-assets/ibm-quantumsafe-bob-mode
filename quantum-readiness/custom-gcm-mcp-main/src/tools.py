...
- Upload JSON file: service="discovery", operation="import.upload", path_params={"profile_identifier": "profile_id_123"}, file_path="/path/to/data.json"
...
For raw API calls, use: method="GET", endpoint="/ibm/usermanagement/api/v1/users\"""",
            inputSchema={
                ...
                "properties": {
                    ...
                    "operation": {
                        "type": "string",
                        "description": "Operation in format 'resource.action' (e.g., users.list, clients.get, import.upload)"
                    },
                    "params": {
                        "type": "object",
                    ...
                    "path_params": {
                        "type": "object",
                        "description": "Path parameters to substitute (e.g., {userId}, {clientId}, {profile_identifier} - use profile ID for import.upload)"
                    },
                    "body": {
                        "type": "object",
                    ...
                    "file_path": {
                        "type": "string",
                        "description": "Local file path for upload operations - must be a JSON file (e.g., /path/to/data.json)"
                    },
                    "method": {
                        "type": "string",
...
async def handle_api(args: Dict[str, Any]) -> List[TextContent]:
    ...
    if endpoint_def.get("upload"):
        file_path = args.get("file_path")
        if not file_path:
            return [TextContent(type="text", text=json.dumps({
                "error": "Missing file_path parameter",
                "hint": "For upload operations, provide file_path with the path to a JSON file"
            }, indent=2))]
        ...
        if not file_path.lower().endswith('.json'):
            return [TextContent(type="text", text=json.dumps({
                ...
                "file_path": file_path,
                "hint": "Only JSON files are supported for upload"
            }, indent=2))]
        
        return await execute_upload(client, path, file_path, endpoint_def.get("file_field", "file"))
...
def _sync_upload(
    ...
    """Synchronous file upload — runs in thread pool to avoid blocking event loop."""
    try:
        response = client.upload(endpoint, file_path, file_field)
...
async def execute_upload(
    ...
    """Execute file upload in a thread pool with timeout, never blocking the event loop."""
    try:
        result = await asyncio.wait_for(
            asyncio.to_thread(_sync_upload, client, endpoint, file_path, file_field),
            timeout=config.GCM_REQUEST_TIMEOUT
        )
        ...
        return [TextContent(type="text", text=json.dumps({
            "error": f"Upload timed out after {config.GCM_REQUEST_TIMEOUT}s",
            "endpoint": endpoint,
            "file_path": file_path,
            "hint": "The file upload did not complete in time. Try again or check file size and network."
        }, indent=2))]
